    ...
    try:
        with connect_database(target, close_on_exit=True) as connection:
            ...
            opportunity_committee_schema = (
                table_exists(
                    connection, "opportunity_committee_mandates"
                )
                if database_dialect(connection) == "postgresql"
                else True
            )
            ...
            portfolio_capital_learning_schema = (
                all(
                    table_exists(connection, table)
                    for table in (
            ...
            alpha_capital_schema = (
                table_exists(connection, "alpha_capital_mandates")
                if database_dialect(connection) == "postgresql"
                else True
            )
            availability_schema = (
        ...
        return {
            "ready": bool(
                ...
                and portfolio_capital_learning_schema
            ...
            "portfolio_capital_learning_schema": bool(
                portfolio_capital_learning_schema
            ),
            ...
            "alpha_capital_schema": bool(alpha_capital_schema),
