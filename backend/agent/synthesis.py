...
def _private_switch_execution_summary(payload: dict[str, Any]) -> dict[str, Any]:
    fields = (
        "selected_code",
        "candidate_code",
        "status",
...
def _private_switch_lifecycle_summary(payload: dict[str, Any]) -> dict[str, Any]:
    fields = (
        "selected_code",
        "candidate_code",
        "status",
...
def _private_context_summary(outputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "portfolio_context": {
            ...
            "fund_switch_execution_reviews": _private_switch_execution_summary(
                context.get("fund_switch_execution_reviews") or {}
            ),
            "fund_switch_lifecycle": _private_switch_lifecycle_summary(
                context.get("fund_switch_lifecycle") or {}
            ),
...
7. 采用以下研究顺序：组合适配与重合风险；底层市场和板块环境；中期趋势与动量及其反转风险；同类相对表现、费用和载体质量；披露持仓与盈利支撑；新闻催化；失效条件；分批执行和复盘。同类平均只用于相对诊断，不得称为可投资替代品；替代审查也不等于赎回指令。候选基金的 durability.decision_gate 未通过时，不得把近期榜单领先描述为可换入机会；通过后仍必须检查 due_diligence。只有 due_diligence.decision_gate.eligible_for_holding_period_cost_review 为 true 时，才可描述为“进入用户持有期成本核验”，不得描述为应当换仓。只有 private_context.portfolio_context.fund_switch_quotes 中对应候选的 status 为 confirmed_current、integrity_verified 与 portfolio_binding_current 均为 true，且 executable_switch_cost_confirmed 为 true 时，才可把成本描述为已确认；其他情况必须把平台报价或到账证据列为未知项。只有 fund_switch_execution_reviews 中对应候选 status 为 ready_for_redemption_review、redemption_review_ready、integrity_verified 与 current_bindings 均为 true 时，才可描述为“可进入人工赎回复核”。无论该状态如何，candidate_purchase_ready 与 execution_authorized 都必须保持 false；申购必须等待真实赎回款到账并重新报价。fund_switch_lifecycle 只有 status 为 ready_for_manual_purchase_review、integrity_verified 为 true 且 execution_authorized 为 false 时，才可描述“真实到账后的申购门禁已通过，可由用户人工复核”，不得改写成买入建议或授权。只有 lifecycle attribution.status 为 available 时，才可描述已经发生的历史增量结果；必须明确它是相对继续持有原基金的历史归因，不是未来收益预测。成本已确认、赎回复核通过、到账后门禁通过或历史归因为正，都不授权自动交易。
