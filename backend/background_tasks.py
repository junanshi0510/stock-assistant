...
@celery_app.task(
    bind=True,
    name=TASK_QUANT_SELECTION_RUN,
    ignore_result=True,
)
def execute_quant_selection_run_job(self, job_id: str):
    """Run one durable point-in-time stock-selection experiment."""
    jobs = BackgroundJobRepository()
    worker_id = _worker_id(self)
    lease_seconds = int(
        os.getenv("QUANT_SELECTION_JOB_LEASE_SECONDS", "2100")
    )
    job = jobs.claim_job(
        str(job_id),
        worker_id,
    ...
    if (
        job.get("queue_name") != QUEUE_MARKET
        or job.get("job_type") != "quant_selection_run"
    ):
        jobs.fail_job(
            str(job_id),
            worker_id,
            error_code="TASK_ROUTE_MISMATCH",
            error_message="quant selection task route mismatch",
            retryable=False,
        )
        return {"job_id": str(job_id), "status": "failed"}
    ...
    if (
        ...
        jobs.fail_job(
            str(job_id),
            worker_id,
            error_code="QUANT_SELECTION_SCOPE_INVALID",
            error_message="quant selection job scope is missing or invalid",
            retryable=False,
        )
        return {"job_id": str(job_id), "status": "failed"}
    try:
        from quant_selection_service import execute_run
        ...
        updated = jobs.fail_job(
            str(job_id),
            worker_id,
            error_code="QUANT_SELECTION_TIMEOUT",
            error_message="quant selection run reached its execution limit",
            retryable=False,
        )
    except BackgroundJobLeaseError:
        return {"job_id": str(job_id), "status": "lease_lost"}
