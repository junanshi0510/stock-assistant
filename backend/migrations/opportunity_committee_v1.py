# -*- coding: utf-8 -*-
"""PostgreSQL migration for immutable strategy committee mandates."""
...
MIGRATION_ID = "opportunity-investment-committee.v1"
MIGRATION_LOCK_ID = 8_561_217_438_902_447
...
def install_opportunity_committee_schema(connection) -> None:
    connection.exec_driver_sql(POSTGRES_DDL)
    connection.exec_driver_sql(POSTGRES_GUARDS)
    trigger = "trg_opportunity_committee_mandates_immutable_pg"
    ...
    connection.exec_driver_sql(
        ...
        "ON opportunity_committee_mandates FOR EACH ROW "
...
def migrate(database_url: str) -> None:
    if not is_postgres_target(database_url):
        raise ValueError(
            "策略投资委员会生产迁移只接受 PostgreSQL DATABASE_URL"
        )
    ...
    try:
        with engine.begin() as connection:
            ...
            install_opportunity_committee_schema(connection)
    finally:
        engine.dispose()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Install Opportunity Investment Committee v1 schema"
    )
    parser.add_argument(
        "--database-url",
        default=(
