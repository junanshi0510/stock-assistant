# -*- coding: utf-8 -*-
"""PostgreSQL schema for immutable multi-horizon Alpha capital mandates."""
...
MIGRATION_ID = "alpha-capital-router.v1"
MIGRATION_LOCK_ID = 7_931_846_220_516_429
...
def install_alpha_capital_router_schema(connection) -> None:
    ...
    trigger = "trg_alpha_capital_mandates_immutable_pg"
    connection.exec_driver_sql(
    ...
    connection.exec_driver_sql(
        ...
        "ON alpha_capital_mandates FOR EACH ROW "
        "EXECUTE FUNCTION stock_assistant_reject_mutation()"
    )
    checksum = hashlib.sha256(POSTGRES_DDL.encode("utf-8")).hexdigest()
    connection.execute(
        text(
            """
...
def migrate(database_url: str) -> None:
    if not is_postgres_target(database_url):
        raise ValueError(
            "Alpha 资本路由生产迁移只接受 PostgreSQL DATABASE_URL"
        )
    ...
    try:
        with engine.begin() as connection:
            ...
            install_alpha_capital_router_schema(connection)
    finally:
        engine.dispose()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Install multi-horizon Alpha capital router v1 schema"
    )
    parser.add_argument(
        "--database-url",
        default=(
