...
本地后端全量回归为 `492 tests` 通过。前端 `npm audit` 返回 `0 vulnerabilities`；Vite `8.1.5` 生产构建成功并转换 `1849` 个模块。当前 OpenAPI 为 `164` 个操作。
...
本节在实际完成 GitHub 推送、发布前备份、生产迁移、服务重启、真实探针、故障/恢复状态机验证、发布后备份和隔离恢复后更新；本地测试结果不代替生产验收。
