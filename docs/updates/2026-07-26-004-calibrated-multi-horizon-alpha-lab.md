...
- 可恢复性提交 `8caaa0f5415b8b874d5049d8fbc58312e74e7437` 已推送 GitHub `main` 并滚动发布；生产构建为 `1861 modules transformed`；
- `8001/8002` 两个副本均报告同一最终 release、`ready=true`、`full_service_ready=true` 和 `alpha_forecast_schema=true`，五条队列深度均为 `0`；
- 两个 API、5 个 Worker、Celery Beat 和 Nginx 共 8 个应用服务均 active，系统失败单元为 `0`；
- 干净公网浏览器冷会话真实完成“登录 → 研究中心 → 概率实验室”，加载后不为空白、不停留在 fallback、不进入错误恢复页；
- 概率实验室标题、历史/前向双重放行说明、弃权义务、空状态、冻结协议、股票 5/20/60 日和基金 20/60/120 净值日表单均真实可见；
- 股票表单默认沪深 300 基准、60 个月窗口、7 天频率和 30 bps 成本；基金表单切换为扣除成本后正收益、120 个月窗口、30 天频率和 50 bps 成本；
- 桌面页面 `scrollWidth == clientWidth`，无全局横向溢出；干净浏览器会话控制台 error 为 `0`；
- 验收只展开和切换表单，没有提交研究项目。数据库最终核对 Alpha program、run、forecast、outcome 均为 `0`，没有把合成数据写入生产；
- 临时验收账户已经 UI 退出并审计化停用，活跃会话为 `0`；
- 自最终副本启动时间起，API、5 个 Worker、Beat 和 Nginx 的 systemd `error` 日志均为 `0`。

### 12.4 最终备份与隔离恢复

- 发布后备份对象：`backups/postgresql/2026/07/stock-assistant-iZn4ai1fm0tr284w21h4kmZ-20260726T093803Z.dump`；
- 私有 OSS 服务端加密：`AES256`；
- 字节数：`2,552,229`；
- SHA-256：`2c817f8fb80b81632548972a6b45356d5acc5dacbcd9d220dfaa7ba55163072a`；
- 本地 dump 与 SHA-256 先通过校验，再上传对象和校验文件；
- 隔离 PostgreSQL 完整恢复成功，核对 `92` 张表、`17` 个迁移标记，临时恢复库随后删除。
