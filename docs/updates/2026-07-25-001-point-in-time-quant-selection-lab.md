...
- OpenAPI：`175` 条路径、`204` 个操作；本功能新增 `7` 条路径、`7` 个操作；
...
下一阶段应把通过门禁的 shadow mandate 接入现有机会收益实验室，以独立的 5/20/60 交易日前瞻批次继续观察因子衰减，再由策略投资委员会决定是否给极小人工研究预算；不能因为一次回测漂亮就扩大资金或自动下单。
