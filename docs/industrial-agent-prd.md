...
1. [文档目的](#1-文档目的)
   - [1.1 本文档中的关键词](#11-本文档中的关键词)
2. [执行摘要](#2-执行摘要)
   - [2.1 产品定位](#21-产品定位)
   - [2.2 核心判断](#22-核心判断)
   - [2.3 首期工业级 Agent V1 的最小闭环](#23-首期工业级-agent-v1-的最小闭环)
3. [当前产品基线](#3-当前产品基线)
   - [3.1 当前工程形态](#31-当前工程形态)
   - [3.2 已有业务能力清单](#32-已有业务能力清单)
   - [3.3 已有测试保护](#33-已有测试保护)
   - [3.4 当前阻断项](#34-当前阻断项)
   - [3.5 必须保留的产品资产](#35-必须保留的产品资产)
4. [产品目标与成功标准](#4-产品目标与成功标准)
   - [4.1 产品目标](#41-产品目标)
   - [4.2 非目标](#42-非目标)
   - [4.3 北极星指标](#43-北极星指标)
   - [4.4 核心产品指标](#44-核心产品指标)
   - [4.5 业务结果指标](#45-业务结果指标)
5. [用户、角色与权限边界](#5-用户角色与权限边界)
   - [5.1 核心用户画像](#51-核心用户画像)
   - [5.2 角色模型](#52-角色模型)
   - [5.3 授权原则](#53-授权原则)
6. [产品原则](#6-产品原则)
   - [6.1 证据优先](#61-证据优先)
   - [6.2 确定性计算优先](#62-确定性计算优先)
   - [6.3 不可用是合法结果](#63-不可用是合法结果)
   - [6.4 真实来源切换必须透明](#64-真实来源切换必须透明)
   - [6.5 用户约束高于机会分数](#65-用户约束高于机会分数)
   - [6.6 研究候选不等于交易指令](#66-研究候选不等于交易指令)
   - [6.7 可恢复与幂等](#67-可恢复与幂等)
   - [6.8 可解释但不暴露隐式推理](#68-可解释但不暴露隐式推理)
   - [6.9 人工确认控制写操作](#69-人工确认控制写操作)
   - [6.10 产品责任边界前置](#610-产品责任边界前置)
7. [产品信息架构与 Agent 体验](#7-产品信息架构与-agent-体验)
   - [7.1 目标信息架构](#71-目标信息架构)
   - [7.2 Agent 首页](#72-agent-首页)
   - [7.3 标准任务交互](#73-标准任务交互)
   - [7.4 标准结果结构](#74-标准结果结构)
   - [7.5 置信度展示](#75-置信度展示)
   - [7.6 关键用户场景](#76-关键用户场景)
8. [功能范围与优先级](#8-功能范围与优先级)
   - [8.1 P0：工业底座阻断项](#81-p0工业底座阻断项)
   - [8.2 P1：Agent V1 用户价值](#82-p1agent-v1-用户价值)
   - [8.3 P2：规模化与持续优化](#83-p2规模化与持续优化)
   - [8.4 P3：需单独立项](#84-p3需单独立项)
...
9. [目标系统架构](#9-目标系统架构)
   ...
   - [9.2 逻辑架构图](#92-逻辑架构图)
   - [9.3 组件职责](#93-组件职责)
   - [9.4 物理部署阶段](#94-物理部署阶段)
   - [9.5 Java 与 Python 的演进边界](#95-java-与-python-的演进边界)
10. [Agent 运行时设计](#10-agent-运行时设计)
   - [10.1 设计选择](#101-设计选择)
   - [10.2 Agent Run 状态机](#102-agent-run-状态机)
   - [10.3 状态机要求](#103-状态机要求)
   - [10.4 结构化计划协议](#104-结构化计划协议)
   - [10.5 执行策略](#105-执行策略)
   - [10.6 结果合成](#106-结果合成)
   - [10.7 模型不可用时的产品行为](#107-模型不可用时的产品行为)
   - [10.8 不保存隐式思维链](#108-不保存隐式思维链)
11. [工具协议与现有能力映射](#11-工具协议与现有能力映射)
   - [11.1 工具注册元数据](#111-工具注册元数据)
   - [11.2 工具风险等级](#112-工具风险等级)
   - [11.3 标准工具输入](#113-标准工具输入)
   - [11.4 标准工具结果信封](#114-标准工具结果信封)
   - [11.5 现有市场能力到工具的映射](#115-现有市场能力到工具的映射)
   - [11.6 现有基金能力到工具的映射](#116-现有基金能力到工具的映射)
   - [11.7 现有组合能力到工具的映射](#117-现有组合能力到工具的映射)
   - [11.8 工具版本规则](#118-工具版本规则)
   - [11.9 禁止的工具模式](#119-禁止的工具模式)
12. [证据账本设计](#12-证据账本设计)
   - [12.1 目标](#121-目标)
   - [12.2 证据类型](#122-证据类型)
   - [12.3 核心字段](#123-核心字段)
   - [12.4 Claim 与 Evidence 绑定](#124-claim-与-evidence-绑定)
   - [12.5 证据链示例](#125-证据链示例)
   - [12.6 数据新鲜度](#126-数据新鲜度)
   - [12.7 来源冲突](#127-来源冲突)
   - [12.8 Evidence UI](#128-evidence-ui)
   - [12.9 证据保留与隐私](#129-证据保留与隐私)
13. [审计链路设计](#13-审计链路设计)
   - [13.1 审计、证据与可观测性的区别](#131-审计证据与可观测性的区别)
   - [13.2 必须审计的事件](#132-必须审计的事件)
   - [13.3 审计事件结构](#133-审计事件结构)
   - [13.4 防篡改要求](#134-防篡改要求)
   - [13.5 回放能力](#135-回放能力)
14. [数据平台设计](#14-数据平台设计)
   - [14.1 数据域](#141-数据域)
   - [14.2 统一证券主数据](#142-统一证券主数据)
   - [14.3 Provider Gateway](#143-provider-gateway)
   - [14.4 数据源选择策略](#144-数据源选择策略)
   - [14.5 数据质量规则](#145-数据质量规则)
   - [14.6 交易日、时区和快照](#146-交易日时区和快照)
   - [14.7 存储分层](#147-存储分层)
   - [14.8 数据授权门槛](#148-数据授权门槛)
15. [策略与决策引擎](#15-策略与决策引擎)
   - [15.1 五层决策结构](#151-五层决策结构)
   - [15.2 策略注册表](#152-策略注册表)
   - [15.3 首期策略族](#153-首期策略族)
   - [15.4 信号与置信度分离](#154-信号与置信度分离)
   - [15.5 机器学习治理](#155-机器学习治理)
   - [15.6 回测表达规范](#156-回测表达规范)
   - [15.7 决策输出模板](#157-决策输出模板)
16. [Agent 记忆设计](#16-agent-记忆设计)
   - [16.1 记忆分类](#161-记忆分类)
   - [16.2 记忆写入原则](#162-记忆写入原则)
   - [16.3 上下文构建](#163-上下文构建)
   - [16.4 记忆冲突与过期](#164-记忆冲突与过期)
17. [安全、隐私与合规要求](#17-安全隐私与合规要求)
   - [17.1 身份与会话](#171-身份与会话)
   - [17.2 数据隔离](#172-数据隔离)
   - [17.3 加密与密钥](#173-加密与密钥)
   - [17.4 文件与 OCR 安全](#174-文件与-ocr-安全)
   - [17.5 Agent 特有安全](#175-agent-特有安全)
   - [17.6 隐私能力](#176-隐私能力)
   - [17.7 产品合规门槛](#177-产品合规门槛)
   - [17.8 安全验证基线](#178-安全验证基线)
18. [核心数据模型](#18-核心数据模型)
   - [18.1 身份与租户](#181-身份与租户)
   - [18.2 投资事实](#182-投资事实)
   - [18.3 Agent 运行](#183-agent-运行)
   - [18.4 工具与模型](#184-工具与模型)
   - [18.5 证据与审计](#185-证据与审计)
   - [18.6 策略与评测](#186-策略与评测)
   - [18.7 ID 与时间规范](#187-id-与时间规范)
   - [18.8 SQLite 迁移规则](#188-sqlite-迁移规则)
19. [Public API 与事件协议](#19-public-api-与事件协议)
   - [19.1 API 版本](#191-api-版本)
   - [19.2 Agent Run API](#192-agent-run-api)
   - [19.3 Claim 与 Evidence API](#193-claim-与-evidence-api)
   - [19.4 用户数据 API](#194-用户数据-api)
   - [19.5 错误协议](#195-错误协议)
   - [19.6 Tool Gateway 内部 API](#196-tool-gateway-内部-api)
   - [19.7 Provider Gateway 内部 API](#197-provider-gateway-内部-api)

### 第三部分：需求、质量与交付体系

20. [详细功能需求](#20-详细功能需求)
   - [20.1 身份与租户（IAM）](#201-身份与租户iam)
   - [20.2 Agent 任务（AGT）](#202-agent-任务agt)
   - [20.3 证据与审计（EVD）](#203-证据与审计evd)
   - [20.4 数据平台（DAT）](#204-数据平台dat)
   - [20.5 投资 Agent 场景（INV）](#205-投资-agent-场景inv)
   - [20.6 组合与导入（PRT）](#206-组合与导入prt)
   - [20.7 策略与模型（STR）](#207-策略与模型str)
   - [20.8 管理与运营（OPS）](#208-管理与运营ops)
21. [非功能需求与 SLO](#21-非功能需求与-slo)
   ...
   - [21.6 一致性](#216-一致性)
   - [21.7 扩展性](#217-扩展性)
   - [21.8 可访问性与响应式](#218-可访问性与响应式)
   - [21.9 国际化与金融口径](#219-国际化与金融口径)
   - [21.10 成本与配额](#2110-成本与配额)
22. [可观测性与故障处理](#22-可观测性与故障处理)
   - [22.1 统一关联 ID](#221-统一关联-id)
   - [22.2 指标](#222-指标)
   - [22.3 日志](#223-日志)
   - [22.4 Trace](#224-trace)
   - [22.5 告警分级](#225-告警分级)
   - [22.6 必备故障手册](#226-必备故障手册)
23. [Agent 评测体系](#23-agent-评测体系)
   - [23.1 评测目标](#231-评测目标)
   - [23.2 黄金任务集](#232-黄金任务集)
   - [23.3 自动评测指标](#233-自动评测指标)
   - [23.4 人工评审](#234-人工评审)
   - [23.5 安全红队](#235-安全红队)
   - [23.6 发布流程](#236-发布流程)
   - [23.7 线上反馈闭环](#237-线上反馈闭环)
24. [前端产品详细设计](#24-前端产品详细设计)
   - [24.1 导航调整](#241-导航调整)
   - [24.2 今日页](#242-今日页)
   - [24.3 Agent 任务页](#243-agent-任务页)
   - [24.4 专业工作区联动](#244-专业工作区联动)
   - [24.5 数据状态组件](#245-数据状态组件)
   - [24.6 表格与批量分析](#246-表格与批量分析)
   - [24.7 移动端](#247-移动端)
25. [CI/CD 与环境治理](#25-cicd-与环境治理)
   - [25.1 环境](#251-环境)
   - [25.2 Pipeline](#252-pipeline)
   - [25.3 数据库迁移](#253-数据库迁移)
   - [25.4 Feature Flag](#254-feature-flag)
   - [25.5 发布回滚](#255-发布回滚)
26. [分阶段迁移路线](#26-分阶段迁移路线)
   - [26.1 总体策略](#261-总体策略)
   - [26.2 阶段 0：基线治理与决策（2 周）](#262-阶段-0基线治理与决策2-周)
   - [26.3 阶段 1：多用户与数据底座（4-6 周）](#263-阶段-1多用户与数据底座4-6-周)
   - [26.4 阶段 2：工具化、证据与审计（4-6 周）](#264-阶段-2工具化证据与审计4-6-周)
   - [26.5 阶段 3：Agent V1 运行时（5-7 周）](#265-阶段-3agent-v1-运行时5-7-周)
   - [26.6 阶段 4：策略工业化与主动服务（4-6 周）](#266-阶段-4策略工业化与主动服务4-6-周)
   - [26.7 阶段 5：公开生产硬化（4-8 周）](#267-阶段-5公开生产硬化4-8-周)
   - [26.8 建议总周期](#268-建议总周期)
27. [Epic 拆分与依赖](#27-epic-拆分与依赖)
   - [EPIC-01 身份与多租户](#epic-01-身份与多租户)
   - [EPIC-02 PostgreSQL 投资事实平台](#epic-02-postgresql-投资事实平台)
   - [EPIC-03 数据源治理与 Provider Gateway](#epic-03-数据源治理与-provider-gateway)
   - [EPIC-04 Tool Gateway](#epic-04-tool-gateway)
   - [EPIC-05 Evidence 与 Audit](#epic-05-evidence-与-audit)
   - [EPIC-06 Agent Runtime](#epic-06-agent-runtime)
   - [EPIC-07 投资 Agent 场景](#epic-07-投资-agent-场景)
   - [EPIC-08 Evaluation 与 ModelOps](#epic-08-evaluation-与-modelops)
   - [EPIC-09 Observability 与 Production Readiness](#epic-09-observability-与-production-readiness)
   - [27.1 关键路径](#271-关键路径)
28. [团队与职责](#28-团队与职责)
   - [28.1 推荐团队](#281-推荐团队)
   - [28.2 决策职责](#282-决策职责)
29. [工业级 Definition of Done](#29-工业级-definition-of-done)
   - [29.1 产品](#291-产品)
   - [29.2 数据](#292-数据)
   - [29.3 工具与计算](#293-工具与计算)
   - [29.4 Agent](#294-agent)
   - [29.5 安全与隐私](#295-安全与隐私)
   - [29.6 质量与运维](#296-质量与运维)
   - [29.7 治理](#297-治理)
...
32. [Agent V1 验收场景](#32-agent-v1-验收场景)
   - [32.1 基金购买前审查](#321-基金购买前审查)
   - [32.2 数据源同时故障](#322-数据源同时故障)
   - [32.3 OCR 金额错位](#323-ocr-金额错位)
   - [32.4 Worker 中断恢复](#324-worker-中断恢复)
   - [32.5 跨用户攻击](#325-跨用户攻击)
   - [32.6 Prompt Injection](#326-prompt-injection)
   - [32.7 历史回放与最新重跑](#327-历史回放与最新重跑)
33. [示例：一次可审计基金 Agent Run](#33-示例一次可审计基金-agent-run)
   - [33.1 用户请求](#331-用户请求)
   - [33.2 结构化意图](#332-结构化意图)
   - [33.3 工具步骤](#333-工具步骤)
   - [33.4 合格结果示意](#334-合格结果示意)
   - [33.5 审计回放](#335-审计回放)
34. [现有代码改造落点](#34-现有代码改造落点)
   - [34.1 保留并演进](#341-保留并演进)
   - [34.2 新增建议目录](#342-新增建议目录)
   - [34.3 第一批代码任务顺序](#343-第一批代码任务顺序)
...
| `fund.alternatives.get` | `/api/funds/alternatives` | R0 | 同策略份额去重后，以真实每日收益持续性形成候选，再用完整费率档案和定期报告持仓重合下界执行第二阶段替换价值门禁 |
