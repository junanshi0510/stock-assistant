...
- 决策顺序固定为“事实完整性 → 已有仓位风险/纪律 → 冻结后的前瞻策略资格 → 月度新增预算 → 单品/权益/行业容量 → 全组合压力情景”。已有仓位出现数据缺口、超限降仓、风险复核或持有逻辑复核时，新增资金被整体抢占并归零。
- 新候选只接受收益实验室中同时通过独立前瞻批次、成本后基准超额、回撤、普通与家族校正置信区间，并已冻结为当前不可变记分卡的策略。实时页面刚刚“过线”但未冻结时仍为观察状态，不能获得金额。
- 全局人工试运行上限取“月度预算、当前组合市值乘策略允许比例、硬上限 5%”三者中的最小值；多个合格策略先等额分配研究袖套，再按各自冻结纸面组合权重分配，不按历史点收益追逐最高赢家。金额继续受允许市场、单品、权益、最坏行业桶和四组说明性压力情景约束；压力越界时用确定性二分缩放寻找仍通过政策的最大研究金额。
...
| 工作区 | 主要能力 |
|---|---|
| 今日决策 | 按“持仓事实 → 可信估值 → 投资政策 → 持有纪律 → 研究证据”执行决策前门禁，再独立展示前瞻验证和执行后复盘；统一接收市场、Agent、机会工厂、收益实验室和组合情景结果，并提供持久任务、定时检查和待处理提醒 |
...
| 机会工厂 | 定义不可变跨市场机会策略，运行持久扫描，查看候选漏斗、同市场五因子评分、硬门槛淘汰、约束后纸面组合；收益实验室自动验证精确 5/20/60 日成本后超额、独立批次、统计置信度和资金资格 |
...
- 当前 OpenAPI 共 `170` 个操作，其中机会工厂提供 17 条策略、运行、纸面组合、收益政策和记分卡路径（20 个操作）。
