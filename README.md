...
| 工作区 | 主要能力 |
|---|---|
| 今日决策 | 按“决策前证据 → 前瞻验证 → 执行后复盘”展示闭环，统一接收持仓/市场、Agent、机会工厂和组合情景结果，并提供持久任务、定时检查和待处理提醒 |
| 我的资产 | 管理真实持仓与逐项纪律、版本化投资政策、交易账本、FIFO/XIRR 复盘、组合数字孪生、反向压力测试和观察清单 |
...
- 当前 OpenAPI 共 `157` 个操作，其中机会工厂提供 11 条策略、运行、纸面组合和观察路径（14 个操作）。
...
| `MASSIVE_API_KEY` | Massive（原 Polygon.io）美股全市场日线、代码搜索和历史行情；推荐的新变量 |
