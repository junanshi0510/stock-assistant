...
- 本地 Alpha/任务专项 `28 passed`、全量后端 `637 passed`、`13 subtests passed`；OpenAPI 为 `196 paths / 226 operations`，前端构建转换 `1861` 个模块，新增动态模块重试专项 `4 tests`，生产依赖审计为 `0 vulnerabilities`。真实浏览器已验证空状态、完整结果、基金/股票表单、桌面和 `390×844` 移动端，页面无横向溢出且控制台告警/错误为 `0`。
- 公网验收捕获到一次瞬时动态模块传输失败会让 React 工作区空白，因此所有顶级工作区和研究工具统一增加三次受限重试、按工作区隔离的错误边界和明确“重新加载页面”恢复入口；切换到未受影响工作区不再被失败模块连带阻断，已保存的服务端事实不会丢失。
- 功能提交 `cb61477` 已推送 GitHub `main` 并完成 `alpha-forecast-lab.v1` 生产迁移与双副本首轮发布。生产 PostgreSQL 已核对 `92` 张表、`17` 个迁移标记、6 张 Alpha 表、8 个不可变触发器、8 个外键和 20 个索引；两个 API 副本、5 个 Worker、Celery Beat、Redis、Nginx 与私有 OSS 均完整就绪。匿名访问返回 `401`，普通用户跨账户读取返回 `404`、维护操作返回 `403`，管理员维护调度成功；临时账户验收后会话均已清零。
- 可恢复性提交 `8caaa0f` 已推送并滚动发布。公网干净冷会话真实完成登录、研究中心、概率实验室、股票/基金预登记表单切换和退出；页面没有全局横向溢出，控制台错误为 `0`，生产没有写入合成 Alpha 数据。最终 release 在两个 API 副本均完整就绪，8 个服务 active、0 个失败单元、发布窗口 0 条 error 日志；发布后私有 OSS AES256 备份 SHA-256 为 `2c817f8fb80b81632548972a6b45356d5acc5dacbcd9d220dfaa7ba55163072a`、`2,552,229` 字节，已隔离恢复核对 `92` 张表/`17` 个迁移标记。
...
- 研究与采集已经解耦：默认 `warehouse_only` 模式只读本地不可变观察，单次研究的供应商调用数固定为 `0`，结果保存仓库版本、快照 SHA-256、逐股票覆盖、采集模式和冲突排除证据。覆盖不足会明确失败，不会在回测途中临时联网补数。
- 管理员可建立 1–5 年额度安全回填计划、暂停/继续/取消并保留历史；Celery Beat 至少每 `65` 分钟只调度一个供应商目标，先补最近交易日，再推进历史计划。`daily_basic` 使用单个交易日的全市场请求，避免按 12 只股票重复消耗额度；财务接口按股票串行采集。
- 同供应商、同股票、同可见日期若出现不同内容哈希，整组观察会从研究中排除；原始观察、同步事件与已完成结果均不可修改或删除。同步事件使用前序哈希链，Worker 使用带到期时间的持久租约，过期 Worker 的迟到响应不能再写入仓库。
- 新增 6 个受认证操作、5 张生产表、PostgreSQL/SQLite 不可变保护、`quant-factor-warehouse.v1` 迁移和 `quant_factor_warehouse_schema` readiness 门禁。普通用户可读取脱敏覆盖摘要；创建计划、手动调度和查看完整运行审计仅限管理员。后端全量 `617 tests`、仓库专项 `13 tests` 通过，前端生产构建转换 `1857` 个模块且依赖审计为 `0 vulnerabilities`。
- 生产首批真实调用确认当前 `daily_basic` 账号为 `5 次/天`，且当天额度已被既有探针用完。调度器因此新增日配额识别：失败批次冷却到下一个上海自然日 00:15 后才重试，最多保留 30 个跨日恢复机会；不会用固定小时重试在同一天耗尽次数。普通供应商错误仍按 65 分钟冷却、最多 3 次。
- 前端新增仓库覆盖、日期范围、冲突、回填进度、最近批次和配额控制面，并在量化实验中提供“只读仓库（推荐）/研究时直连供应商”显式选择。OpenAPI 为 `188` 条路径、`218` 个操作；本地真实浏览器已验证桌面端与 `390px` 手机宽度无横向溢出、控制台错误为 `0`。
- 功能提交 `9e76af7` 与配额修复 `29257f1` 已推送并发布到 `http://8.148.67.79/` 双副本；两副本完整就绪、六个 Worker/Beat active、发布窗口错误日志为 `0`。生产已创建 `783` 个工作日的三年估值回填计划，首批真实请求因 `5 次/天` 额度耗尽而保留失败审计，并安全冷却至 `2026-07-27 00:15 +08:00`，没有重复消耗当日额度。最终私有 OSS AES256 备份 SHA-256 为 `b6c8bce8b1e62238f70d497d10bfa768acb38827d1baea5cc450266ac3d6eebc`，已隔离恢复核对 `86` 表/`16` 迁移。
- 该功能解决历史财务前视、API 额度被回测重复消耗和采集不可追溯问题；它提高研究可信度与可重复性，不预测必涨/必跌、不连接券商、不自动下单，也不保证盈利。完整设计、数据契约和验收记录见 [`docs/updates/2026-07-26-003-point-in-time-factor-warehouse.md`](docs/updates/2026-07-26-003-point-in-time-factor-warehouse.md)。

### 2026-07-26：A 股可运行量化基准与最低权限研究层
...
- 新增默认“A 股价格多因子研究池（可直接运行）”：冻结 12 只高流动性样本，使用真实价格、趋势、波动和流动性，不调用 Tushare 财务或历史成分接口。它可立即完成真实研究，但因当前名单存在幸存者偏差，固定为 `research_only`，不能自动晋级。
- 真实云端整链探针发现，先并发尝试低额度 `pro_bar/adj_factor` 会形成调用风暴；而当前 `daily_basic` 虽能单次调用，实际只有每分钟/每小时 1 次，无法即时加载 12 只股票的历史估值。A 股冻结研究池现使用独立 `a_share_research` 路径，价格按 BaoStock → 腾讯证券 → 东方财富读取；估值预设单独标为“需批量额度或历史缓存”，不再伪装成基础权限下可直接运行。
...
- “量化选股”新增固定日历研究计划：创建时一次性冻结完整策略、因子权重、市场时区和未来 `6–24` 个按月/按季批次。到期批次不能跳过，未通过、运行失败和计划终止都保留，不允许事后只留下表现好的样本。
- Celery Beat 每 15 分钟核对到期与运行中批次；每个批次独立创建历史时点量化 Run。12 项研究门槛未全部通过时永久记为 `research_only`；全部通过才自动冻结 shadow mandate 并接入既有 5/20/60 交易日前向验证，仍不连接券商、不计算股数、不自动下单。
- A 股新增“披露日财务质量”和“时点估值”两个透明因子。财务指标只允许在 `ann_date <= signal_date` 时出现，报告期不能冒充可见日；估值只允许使用 `trade_date <= signal_date` 的 PE/PB。旧报告的晚到修订不会覆盖更新报告期，同一公告日无法排序的歧义修订会被排除。
- 财务质量由 ROE、毛利率、经营现金流/营业收入和资产负债率组成；至少 3 个分项可用才计算，并按剩余绝对权重重新归一，缺失值不会补零。亏损公司的空 PE 使用明确惩罚而不是直接删样本，避免偷偷形成盈利筛选。
- 只有实际启用的财务因子才请求对应 Tushare `fina_indicator` 或 `daily_basic` 接口。因子证据展示来源、严格时点校验、逐股票失败、歧义修订和信号截面覆盖；启用财务因子时还必须通过时点证据与至少 80% 覆盖门禁。
- 新增 5 个受认证操作、4 张租户/用户隔离的计划/批次/事件表、前序哈希事件链、策略/日历/确认/结果 SHA-256、SQLite/PostgreSQL 不可变保护、`quant-research-program.v1` 迁移、独立 readiness 门禁和持久调度路由。当前 OpenAPI 为 `182` 条路径、`212` 个操作。
- 本地全量回归 `597 passed`、`13 subtests passed`；前端生产构建完成 `1857` 个模块转换，生产依赖审计 `0 vulnerabilities`。真实浏览器从 UI 创建 6 个未来批次，桌面端与 `390×844` 移动端均无页面级横向溢出，控制台错误为 0。
- 该功能减少的是回测挑时点、财务前视和只保留赢家造成的自我欺骗，不提供“必涨/必跌”结论，也不保证未来盈利。完整方法、数据模型、接口和验收记录见 [`docs/updates/2026-07-26-001-preregistered-quant-research-program.md`](docs/updates/2026-07-26-001-preregistered-quant-research-program.md)。
...
- 功能提交 `28f63c3` 已推送 GitHub `main`，完成第 14 个生产迁移并原子滚动发布到 `http://8.148.67.79/` 的 `8001/8002` 双副本；两副本 release、OpenAPI 和 readiness 一致，6 个 Worker/Beat 正常，0 个失败单元、0 条发布后 error 日志。备份脚本工作目录加固提交 `67503cb` 也已推送并在云端从项目根目录实测通过。
- 云端隔离 PostgreSQL 恢复克隆真实完成“接入 → 幂等复用 → 下一开盘 → 60 日结算 → 20 日收集门禁 → 篡改拒绝”后整库销毁；临时普通账户验证认证、用户隔离与 `404` 边界后已停用，会话为 0，认证审计链 `115` 个事件通过。生产业务表未写入合成前向记录。
- 发布前私有 OSS AES256 备份 SHA-256 为 `2b864809911254513440685b44ab79961a00046466611250a64dae717acb5382`，恢复为 `76` 张表、`13` 个迁移标记；最终发布后备份 SHA-256 为 `d02f376f9d9f880d0eee6a4a98576573cddeabe77e0bb58e61883dd3281298c9`，恢复为 `77` 张表、`14` 个迁移标记。
- 本功能只修复“回测结果如何进入真实、可审计的前向证据链”，不预测必涨股票、不生成股数、不连接券商、不自动下单，也不保证盈利。
...
- A 股专业模式逐月读取 Tushare `index_weight`，支持沪深 300、中证 500 和中证 1000；每个信号日只使用当时已生效的最近快照。A/H/美股也可冻结 6–40 只研究池，但没有专业历史成员序列时始终保留幸存者偏差警告，不能升级为纸面策略。
- 当前固定使用中期动量、趋势质量、低波动和流动性四个透明因子；只使用信号日及此前数据做横截面百分位排名，不在样本外结果出来后自动挑权重或“赢家”。组合支持等权、逆波动和“综合分 × 逆波动”，并受最低现金、单股上限、流动性和最多持仓约束。
- 收盘形成目标，最早在后续交易日开盘成交；复权价格只用于信号，未复权开盘和成交额用于执行与容量。逐日账本支持先卖后买、佣金、滑点、卖出税、成交量参与上限、二次容量冲击、部分成交、停牌/零量拒单和订单超时。
- 结果同时展示成本后策略/基准曲线、最新目标篮子、逐因子得分、非重叠样本外窗口、Rank IC、完整成交摩擦和成本翻倍压力。10 项门禁必须全部通过才可冻结内部 shadow mandate；任何失败都固定为 `research_only`，且始终不连接券商、不生成股数、不授权下单。
...
- 后端全量回归 `577 passed`、`13 subtests passed`；前端 Vite 生产构建完成 `1857` 个模块转换，生产依赖审计 `0 vulnerabilities`。真实浏览器从 UI 完成 6 只美股、36 个月实验，生成 641 个交易日、119 笔成交、5 个非重叠样本外窗口；公开降级行情未达到专业双源门槛时正确保持“仅研究”。
...
- 功能与限流加固提交 `87857cd`、`620efee`、`6d3e8e4` 已推送 GitHub `main`，最终 release `6d3e8e4` 原子滚动部署到 `http://8.148.67.79/` 的 `8001/8002` 双 API 副本；两个副本均为 `full_service_ready=true`、`quant_selection_schema=true`，6 个 Worker/Beat 正常，云端发布包专项 `45 tests` 通过。
- 生产隔离普通账户使用现有 Massive Key 完成 8/8 美股专业复权日线、375 个组合交易日、86 笔撮合、3 个非重叠窗口和 17 次 Rank IC；冷启动约 107 秒，Worker 缓存复跑约 6 秒。跨用户读取返回 `404`，未过门禁的 shadow mandate 返回 `409`，3 类数据库篡改均被拒绝；2 个临时账户随后停用、活跃会话为 0，认证审计链 `111` 个事件通过。
- 现有 Massive 数据表现与 Basic 的“5 次/分钟、2 年历史”边界一致，因此专业复权覆盖为 100%，但独立未复权覆盖为 0%、只有 3 个样本外窗口，系统正确保持 `research_only`；若要让美股研究具备通过 4 窗口门禁的基础，当前官方 Starter 提供 5 年历史和不限 API 调用。生产 Tushare Token 已配置，但 `index_weight` 权限尚未开通，A 股历史指数成分模式会明确失败而不是伪造名单。
- 发布前私有 OSS AES256 备份 SHA-256 为 `53bdafece45298c6d79ec0966db834770ea9447e0b02d1a136c01566db9d8af8`，隔离恢复核对 `73` 张表、`12` 个迁移标记；发布后备份 SHA-256 为 `0db20f4b88ce293208b4c948d331a7dc13f825a062c0d21ce866e6276ba4bf80`，恢复核对 `76` 张表、`13` 个迁移标记。
...
- 每个纸面组合由调度器自动追加真实行情观察；A 股、港股、美股分别使用 `000300.SH` 沪深 300 指数、`02800`、`SPY` 作为相对收益基准，并按冻结权重计算 5/20/60 个真实交易日的精确结果。即使中途停机后补跑，也不会把更晚日期冒充目标窗口；相同行情截面使用幂等键去重。
...
- 当前 OpenAPI 共 `212` 个操作，其中机会工厂提供 23 条策略、运行、纸面组合、收益政策、记分卡、市场状态和委员会路径（28 个操作）。
