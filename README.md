...
- 新增 6 个受认证操作、5 张生产表、PostgreSQL/SQLite 不可变保护、`quant-factor-warehouse.v1` 迁移和 `quant_factor_warehouse_schema` readiness 门禁。普通用户可读取脱敏覆盖摘要；创建计划、手动调度和查看完整运行审计仅限管理员。后端全量 `616 tests`、仓库专项 `12 tests` 通过，前端生产构建转换 `1857` 个模块且依赖审计为 `0 vulnerabilities`。
