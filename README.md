...
### 2026-07-13：基金收益率历史重现

- 新增版本化指标 `fund_return_recurrence@1.0.0`，基于基金真实确认净值计算近 1 月、3 月、6 月和 1 年滚动收益率。
- 对每个周期展示当前收益率、当前同水平区间起点，以及上一个独立同水平区间的日期、收益率、差值、相隔自然日和净值样本数。
- “同水平”采用可披露的动态容差：最低 `0.20` 个百分点、最高 `2.00` 个百分点，并随当前收益率绝对值调整；前端同时展示实际容差。
- 算法跳过当前连续命中区间，并要求上一次记录至少再间隔 5 个净值样本，避免把昨天或同一波行情误报成历史重现。
- 历史值未落入容差时只标记“历史最近似”，明确展示差异，不会将最近似值伪装成曾达到相同收益率；历史长度不足时明确显示不可用。
- 基金中心和 Agent 研究结果共用同一确定性计算结果；Agent 将完整指标保存为 Evidence，支持来源、版本、哈希和审计链核验。
- 增加独立区间、最近似降级、短历史拒绝、四周期输出和 Agent Evidence 绑定测试，并使用真实基金 `001480` 的 60 个月净值验证。
...
| 基金中心 | 基金发现与搜索、真实净值分析、滚动收益率历史重现、盘中估值、回撤与恢复、同类排名、替代品、分红、定期报告持仓、披露变化、多基金比较与重合度 |
...
- 近 1 月、3 月、6 月和 1 年滚动收益率的历史重现时间、独立区间和最近似差异。
...
backend/
  ...
  strategies/
    ...
    fund_return_recurrence.py    基金同周期收益率历史重现指标 1.0.0
  routers/
