...
- 机会工厂：版本化候选池、持久扫描、同市场多因子、硬门槛、约束后纸面组合和冻结后的前瞻观察；不拥有交易权限，也不代表交易所全量扫描。
...
- 投资任务：确定性决策条件同步为用户级持久任务。用户确认不等于风险解决；只有完整的新证据不再触发条件时才能自动转为 `resolved`，全部状态变化进入不可变事件链。
...
`backend/routers/portfolio.py`、`backend/routers/opportunities.py` 和 `backend/routers/agent.py`。
...
`backend/hot_stocks.py` 是热门榜供应商路由边界。A 股和港股首先使用 Tushare Pro 最近完整交易日日线，美股首先使用 Alpha Vantage 官方三榜；东方财富/Yahoo 只能作为带 `public_fallback` 和 `degraded=true` 的 best-effort 降级源，新浪不在路由中。一次市场级 bundle 同时返回所需榜单、来源等级、截止时间、时效、方法和逐供应商尝试，机会工厂与市场日报不得再按榜单种类重复请求。连续失败进入进程内短期熔断，所有异常在写入状态、日志或响应前必须脱敏。`GET /api/market/providers` 只报告配置布尔值与运行摘要，不主动探测，也不暴露任何凭据。
...
- PostgreSQL 是生产唯一事实源，保存用户、持仓、交易、Agent、机会策略/运行/纸面观察、Evidence、任务载荷、租约、结果哈希和审计事件。应用启动不得自动运行 SQLite DDL；缺少迁移表时拒绝启动。
...
- `observability.py` 输出带 request/task/run/job 关联 ID 的 JSON 日志和 Prometheus 指标，并隐藏凭据；`health.py` 同时检查 PostgreSQL、Redis、OSS 和全部队列 Worker。
...
`frontend/src/api/portfolio.js`、`frontend/src/api/opportunities.js` 和 `frontend/src/api/auth.js`，共享的 Cookie/CSRF 与 HTTP 错误处理放在
`frontend/src/api/client.js`。浏览器不得把会话 Token 写入 `localStorage` 或传给业务组件。
机会工厂位于 `frontend/src/tabs/OpportunityTab.jsx`，策略编辑、扫描结果和纸面跟踪分别由 `features/opportunities/` 下的独立组件承担。页面必须同时展示候选范围、策略版本、数据源/数据日、因子覆盖率、硬门槛原因、组合限制和纸面跟踪限制；不得只展示一个不透明总分或把“入围”渲染成买入指令。宽表只允许在自身容器内滚动，不能扩大手机页面宽度。
...
- 专业热门榜与公开网页降级源必须明确分层；未配置、授权不足、熔断、陈旧缓存和公开降级不得显示为“专业实时”。供应商状态接口不得回传 Key，也不得为了健康检查额外消耗行情额度。
- 机会扫描不得把预设、自选或热门榜称为交易所全量；每次结果必须保存实际候选来源、截断数、失败来源和未解决限制。
- 股票因子缺失不得静默重分配权重；同市场候选池分位不得包装为行业中性或全市场分位，综合分也不得覆盖数据、风险或基本面硬门槛。
- 纸面组合只能从冻结后的真实行情追加观察，不得回填历史表现、重写基准、推断真实成交或把行情失败股票的权重分配给成功股票。
