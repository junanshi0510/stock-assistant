...
- PostgreSQL 是生产唯一事实源，保存用户、持仓、交易、不可变市场观察与组合估值、量化组合运行/事件/纸面指令、Agent、机会策略/运行/纸面观察、收益政策与记分卡、组合资金计划、组合数字孪生运行、Evidence、任务载荷、租约、结果哈希和审计事件。应用启动不得自动运行 SQLite DDL；缺少迁移表时拒绝启动。
